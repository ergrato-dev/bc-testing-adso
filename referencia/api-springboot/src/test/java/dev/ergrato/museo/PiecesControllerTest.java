...
class PiecesControllerTest {
    ...
    @Test
    @DisplayName("should respond 200 with all pieces")
    void shouldRespond200WithAllPieces() throws Exception {
        when(service.list()).thenReturn(List.of(new Piece("Guernica", "Picasso", 1937)));

        mockMvc.perform(get("/api/pieces"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].artist").value("Picasso"));
    }

    @Test
    @DisplayName("should respond 200 with the piece when it exists")
    void shouldRespond200WhenPieceExists() throws Exception {
        when(service.get(1L)).thenReturn(new Piece("Guernica", "Picasso", 1937));
    ...
    @Test
    @DisplayName("should respond 204 when deleting a piece")
    void shouldRespond204WhenDeletingAPiece() throws Exception {
